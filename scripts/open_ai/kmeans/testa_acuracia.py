...
def main() -> None:
    ...
    kmeans_artifact = run.use_artifact('mlops2023-2-org/wandb-registry-model/kmeans:latest', type='model')
    downloaded_model_dir = kmeans_artifact.download()

    # Verificar e imprimir o caminho do modelo
    downloaded_model_path = os.path.join(downloaded_model_dir, 'kmeans.pkl')
    ...
    scaler_artifact = run.use_artifact('tcc-ufrn/k-means-model/scaler:latest', type='preprocessing')
    ...
    predicted_labels = kmeans.predict(matrix_scaled)
    logging.info(f"Tamanho de predicted_labels: {len(predicted_labels)}")

    mapeamento = ["Vagas", "Ciências", "Eventos", "Informes"]
    ...
    image_path = '../../../images/confusion_matrix_kmeans.png'
    ...
    image_path = '../../../images/report_kmeans.png'
    plt.savefig(image_path)
